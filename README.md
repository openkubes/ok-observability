...
The stack is installed per cluster by ok-cluster's gated `make install-observability` (OK-79): it deploys the `ok-observability-standard` profile and runs the contract test, failing unless all five guarantees pass. Admin credentials are read from a Kubernetes Secret `ok-observability-credentials` (Grafana `admin.existingSecret`, OpenSearch `secretKeyRef`, Fluent Bit `${OPENSEARCH_PASSWORD}`) — no plaintext passwords are passed to Helm. Provider Values (`*.provider-values.yaml`) are git-ignored — never commit a filled-in copy. For the schema, copy the committed [`provider-values.example.yaml`](./provider-values.example.yaml) to `<cluster>.provider-values.yaml` and fill it in; for a throwaway/verify cluster any self-consistent values work (the same file seeds OpenSearch's `INITIAL_ADMIN_PASSWORD` and the Grafana/Fluent-Bit side, so they always match). Datacenter-envelope clusters instead source that Secret from the central Vault on ok-shared via a `VaultStaticSecret` — see [`implementations/vault-secrets-operator/`](./implementations/vault-secrets-operator/) (ADR-Platform-025, stood up in OK-110, wired in OK-109 Part 2; proven on ok-robotics). Same Secret, same keys, no chart change — only *who populates it* differs. Edge/air-gapped clusters keep this offline file profile (Secret Contract, OK-71).
